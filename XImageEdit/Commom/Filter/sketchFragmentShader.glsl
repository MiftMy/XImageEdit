...
uniform float singleStepOffsetX;
uniform float singleStepOffsetY;
...
void main()
{
    ...
    for(int i = -2; i<=2; i++)
    {
        for(int j = -2; j<=2; j++)
        {
            float x = textureCoordinate.x+singleStepOffsetX*float(i);
            float y = textureCoordinate.y+singleStepOffsetY*float(j);
            vec4 tempColor = texture2D(inputImageTexture, vec2(x,y));
    ...
    gl_FragColor = vec4(vec3(result,result,result), oralColor.a);
}
